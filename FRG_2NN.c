...
diverge_model_t* MODEL( index_t nk, index_t nkf , double U); //This is defined under the main function.
...
int main( int argc, char* argv[] ) {
    ...
    char modelname[80];
    // Format the string
    sprintf(outputname, "2NN_%d_%d_%.2f_%.2f_out.dvg", (int)nk,(int)nkf,U,mu);
    sprintf(modelname, "2NN_%d_%d_%.2f_%.2f_mod.dvg", (int)nk,(int)nkf,U,mu);
    ...
    diverge_model_t* m = MODEL(nk,nkf,U);

    //This needs to be big enough that the results don't change. 
    double tu_form_factor_cutoff = form;
    ...
    eu.maxvert = 200.0; //This defines the max value, 50 is default, but large U requires larger value. 
    double cmax[3] = {0}; 
    double vmax = 0.0;
...
diverge_model_t* MODEL( index_t nk, index_t nkf , double U) 
{
    diverge_model_t* m = diverge_model_init();
    strcpy(m->name, __FILE__ );

    double a_lat = 1.41421356;
    double b_lat = 1.41421356;
    double c_lat = 1;
    ...
    m->positions[0][0] = 0.25 * a_lat;
    m->positions[0][1] = 0.25 * b_lat;
    m->positions[0][2] = 0.0;


    m->positions[1][0] = 0.75 * a_lat;
    m->positions[1][1] = 0.75 * b_lat;
    m->positions[1][2] = 0.0;
    /*
    ...
    */
    ...
    m->hop = diverge_read_W90_C("2NN_hr.dat",0,&m->n_hop, &m->n_orb);
