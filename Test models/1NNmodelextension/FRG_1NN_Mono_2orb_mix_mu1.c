...
int main( int argc, char* argv[] ) {
    ...
    double J = atof(argv[2]);  // maybe change to mu 
    ...
    char modelname[80];
    // Format the string
    sprintf(outputname, "1NN_mono_mix_mu1_%d_%d_%.2f_%.2f_out.dvg", (int)nk,(int)nkf,U,J);
    sprintf(modelname, "1NN_mono_mix_mu1_%d_%d_%.2f_%.2f_mod.dvg", (int)nk,(int)nkf,U,J);
    ...
    double tu_form_factor_cutoff = 4.00;
...
*/
    ...
    m->hop = diverge_read_W90_C("1NN_Mono_2orb_mix_hr.dat",0,&m->n_hop, &m->n_orb);
    ...
    sites[0].function[0] = orb_dz2;
    
    sites[1].n_functions = 1;
    sites[1].amplitude[0] = 1.;
    sites[1].function[0] = orb_dx2y2;
/*
