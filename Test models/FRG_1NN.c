...
int main( int argc, char* argv[] ) {
    ...
    char modelname[80];
    // Format the string
    sprintf(outputname, "NAME_%d_%d_%.2f_%.2f_out.dvg", (int)nk,(int)nkf,U,mu);
    sprintf(modelname, "NAME_%d_%d_%.2f_%.2f_mod.dvg", (int)nk,(int)nkf,U,mu);
...
diverge_model_t* MODEL( index_t nk, index_t nkf , double U , double mu) 
{
    ...
    */
    ...
    m->hop = diverge_read_W90_C("1NN_hr.dat",0,&m->n_hop, &m->n_orb);
        ...
        for (index_t o1 = 0; o1 < n_orbitals_per_atom; ++o1)
        {
            for (index_t o2 = 0; o2 < n_orbitals_per_atom; ++o2)
            {
                ...
                else
                {
                    ...
                    m->vert[m->n_vert++] = (rs_vertex_t){.chan='C',.s1=-1, .R={0,0,0}, .o1=o1+n_orbitals_per_atom*atom1, .o2=o2+n_orbitals_per_atom*atom1, .V= };
