...
def suscepPlot(mod,out,channel="P",title=""):
    ...
    #THIS ONLY WORKS FOR La3Ni2O7!
    dz2 = np.shape((ibz_susz[:,0,0]))[0]*[0]
    dx2y2 = np.shape((ibz_susz[:,0,0]))[0]*[0]
    for i in range(int(no/2)):
        dz2 += np.abs(np.real(ibz_susz[:,2*i,2*i]))
        dx2y2 += np.abs(np.real(ibz_susz[:,2*i+1,2*i+1]))
    ax.plot(steps,dz2,c="purple")
    ax.plot(steps,dx2y2,c="orange")
    #ax.scatter(steps,dz2,c="black",marker="o")
    #ax.scatter(steps,dx2y2,c="black",marker="o")

    print(susz.shape)
    dz2_points = np.zeros((susz.shape[2]))
    dx2y2_points = np.zeros(susz.shape[2])
    for i in range(int(no/2)):
        dz2_points += np.abs(np.real(susz[2*i,2*i,:]))
        dx2y2_points += np.abs(np.real(susz[2*i+1,2*i+1,:]))

    ax.plot(steps[where_on_path],dz2_points[k_idx],c="black",marker=".",linestyle="None")
    ax.plot(steps[where_on_path],dx2y2_points[k_idx],c="black",marker=".",linestyle="None")
...
def main():
    ...
    mod.klabels=["G","M","X","G","Z","A","R","Z"]

    out = do.read(f"1NN_{nk}_{nkf}_{U}_{mu}_out.dvg")

    plotBandStructure(mod,save=False)

    suscepPlot(mod,out,channel="P",title=f"{nk}_{nkf}_{ff}_{U}_{mu}")
    suscepPlot(mod,out,channel="C",title=f"{nk}_{nkf}_{ff}_{U}_{mu}")
    suscepPlot(mod,out,channel="D",title=f"{nk}_{nkf}_{ff}_{U}_{mu}")
    plt.show()
    exit(0)
    #mod = do.read("Sr2RuO4_32_4_1.20_0.25_2.01_mod.dvg")
    #out = do.read("Sr2RuO4_32_4_1.20_0.25_2.01_out.dvg")

    #suscepPlot(mod,out,channel="P",title="32_4_2.01")
    #suscepPlot(mod,out,channel="C",title="32_4_2.01")
    #suscepPlot(mod,out,channel="D",title="32_4_2.01")
    #plt.show()

    #plot_SCgap(out,mod,"P",out.Pvec[0],out.Pval[0],real_space=False)
    #save_SCgap(out,mod,"P",out.Pvec[0],gap_file=f"gap_file_{nk}_{nkf}_{U}_{J}_{ff}.txt")
