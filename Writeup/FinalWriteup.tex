...
\medskip
\noindent The complete phase diagram for the values discussed above is shown in Fig.\ref{fig:1NNpd}. This particle-hole symmetric phase diagram shows a magnetic dome sandwiched between two superconducting regions. Moreover, narrow magnetic stripes appear for even
integer chemical potential values. There are patches of a charge density wave instability sorrounding the superducting regions and a competition between the Magnetism and Superconductivity is observed. This later observation
is what is expected from the Hubbard model and the cuprate phase diagram \cite{kivelson1998electronic,fradkin2015colloquium,vanhala2018dynamical}.\par

\medskip

\noindent Despite having outlined serveral limitations that this method presents, it is nevertheless 
informative to compare the results presented here with that of the Cuprate ph
...
Recent findings have claimed that the 2D Hubbard model (same model as the 1NN model discussed here) does not have a superconducting ground state\cite{qin2020absence}. 
The analysis in Ref. \cite{qin2020absence} is conducted in a slightly different framework than the one used in this project: using DMRG \cite{white1992density} at moderate-strong coupling and for values of U between 6-8eV\textit{(which is claimed to be the regime relevant to the cuprates)}.
...
a stabilised superconducting region is found. The superconducting order parameter is plotted on top of the Fermi-Surface for the first BZ for all calculated points in the Superconducting region (Fig.\ref{fig:1NNNSC}a).
The order parameter shows antisymmetry with respect to a 90 degree rotation and therefore
the superconducting region is d-wave symmetric. This is in agreement with what is expected from the Cuprate superconductors \cite{tsuei2000pairing}. 
