...
Due to the overlap between the quantum mechanical wavefunctions that describe these orbitals, electrons are allowed to 'hop' to neighbouring sites. The probability that this hopping process will occur is given by a tunnelling amplitude, which can be calculated using a hopping integral. \par
\medskip
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    ...
    \caption{\textbf{Two-Dimensional Tight-Binding Models:} Three pannels showing the tight binding models for the 1NN, 1NNN and 1NN2 models discussed in Section~\ref{subsec:1NNModel},~\ref{subsec:1NNNModel} and~\ref{subsec:1NN2Model} respectively. Panel a) shows the nearest-neighbour hopping case, where \textit{t} depicts the hopping amplitude between the neighbouring sites. Panel b) shows the inclusion of the next-nearest neighbour hopping, the magnitude given by \textit{t'}.
...
This cutoff is then interpolated between the solveable initital state and the full path integral solution by subsequently including electronic interactions. For a spin-independent system this would transform the 
...
are beyond the scope of this project it is more convenient to work with the effective action than it is to do so with the partition functional.) \footnote{If interested in why see Ref. \cite{metzner2012functional}}.}
...
\begin{minipage}[t]{0.48\textwidth}
    ...
    maxima of each plot in order to allow for the clear vizualisation of the trend as $\mu$ is varied.  }
