...
These are condensates of cooper pairs formed by a \textbf{different} pairing mechanism than the electron-phonon coupling predicted by BCS theory.
However, for some materials there is no wide agreement on what the mechanism otherwise is. 
...
\noindent In the case of unconventional superconductors, the form of the effective pairing interaction $\Gamma(k,k')$ has remained as an unanswered question for decades.
...
\cite{migdal1958interaction} :

\begin{equation}\label{Pairing interaction SF}
    \Gamma(k,k') = \frac{3}{2} U^2 \chi^S(k-k') -\frac{1}{2}U^2 \chi^C(k-k') + U
\end{equation} 

\noindent where U is the on-site Coulumb repulsion and $\chi^S$, $\chi^C$ are the interacting spin-susceptibilities in the Charge (C) and Spin(S) channel respectively. The later are given below.
...
\noindent The calculations presented in this project are carried out in the spin-fluctuation framework. This theory has succesfully captured key features in phase diagrams of unconvential superconductors but also has its limitations.
The most relevant example is that of the cuprate phase diagram.
Spin-fluctuation theory is able to describe the superconducting dome and correct order parameter \cite{moriya2006developments, scalapino1995case} and fails
...
This model has been widely studied since it resembles the structure of the cuprate high-temperature superconductors (REFERENCE). 
...
The flow equation derived above can be solved for a limited amount of systems (REFERENCE). However, in most cases, the memory demand for the 
...
Partitions of unity \footnote{A detailed explanation of what these are can be found in} are then introduced into a specific part of the flow equation. This reduces an otherwise computationally expensive nested integral to a matrix product. 
...
the characteristic Mott insulating phase of the Cuprate phase diagrams (REFERENCE), which limits how well the results presented in this project can be directly compared with existing literature. 
Nevertheless, it manages to successfully capture the competition between Magnetic and Superconducting instabilities, which is one of the main focus of the results presented here. 
...
The general aim is to solve the Hubbard-(Kanamori) model for the 2D Hubbard model. This is achieved computationally, using the Diverge package. This package implements the TU$^2$FRG
...
Diverge and some of the post-processing techniques used. 
...
exisiing literature (REFERENCE), a $d_{x^2-y^2}$ and $d_{3z^2 -r^2}$ orbital are including. 
Their respective hopping parameters are calculated using the  table of interatomic matrix elements calculated by J.C. Slater and G.F. Koster\cite{slater1954simplified}. 
In this calculation, the values of the $\sigma,  \pi, \delta$ bond strength  are approximated to  $\approx  1, 0.5, 0.05eV$ respectively in order to capture their relative values (REFERENCE).
