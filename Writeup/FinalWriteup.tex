...
These are condensates of cooper pairs formed by a \textbf{different} pairing mechanism than the electron-phonon coupling predicted by BCS theory. Whilst there exists consensus that their superconductivity does not in fact stem from the BCS mechanism, 
for some materials there is no wide agreement on what the mechanism otherwise is. 
...
\noindent where U is the on-site Coulumb repulsion and $\chi^S$, $\chi^C$ are the interacting spin-susceptibilities in the Charge (C) or Spin(S) channel respectively. The later are given below.
...
\noindent The interacting spin susceptibilities are expressed in terms of the non-interacting dynamic spin susceptibility which is stated below without formal proof \cite{moriya2000spin}. 

\begin{equation}\label{chi 0}
    \chi_{ps}^0(q, i \omega) = -\sum_{k} \int_{0}^{\beta} d\tau G^0_{ps}(k+q \tau) G^0_{sp}(k, -\tau)e^{i\omega \tau}
\end{equation} 

\noindent The calculations presented in this project are carried out in the spin-fluctuation framework. This theory has succesfully captures key features in phase diagrams of unconvential superconductors but also has its limitations.
