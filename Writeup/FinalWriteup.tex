...
for the 2D Hubbard-(Kanamori) model, for which the divERGe package is used. This package implements the TU$^2$FRG
...
the form factor value was set at 4\AA\footnote{This is equivalent to a number of form factor shells of 4 since the lattice spacing of the models here is set to 1\AA.} after the convergence of  
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    ...
    \caption{\textbf{Convergence testing:} \textit{Panel a)} Transition temperature as a function of form factor cut-off value for the nearest-neighbour 2D Hubbard model, $U$ = 5.00, $\mu$ = 0.20eV. \textit{Panel b)} Time taken to complete the calculation
...
enhances the transition temperature \cite{maple1995interplay,sun2016dome}. Moreover, the magnitude of the Coulomb repulsion and Tc are positively correlated. 
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    \centering
    \includegraphics[width=0.5\textwidth]{1NNstripes.png}  % Adjust width as needed
    \caption{\textbf{Magnetic stripe in the 1NN model for doping at $\frac{1}{8}$ of the bandwidth}: \textit{Panel a)} Transition temperature plotted in a logarithmic scale as a function of Coulomb repulsion ($U$) along the Magnetic stripe at $\mu$ =1eV. 
...
next-nearest-neighbour is introduced. Additionally, general trends in the superconducting transition
temperature ($T_c$) are also observed: it is maximized closest to the 
magnetic instability, and it increases as a function of $U$. Finally, the emergence of magnetic stripes is still observed. \par
\medskip
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    \centering
    \includegraphics[width=0.70\textwidth]{1NNNSC_075.png}  % Adjust width as needed
...
change several of properties of the superconducting region.  Proximity to a Magnetic instability further enhances $T_c$.
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    \centering
    \includegraphics[width=0.50\textwidth]{tprimecont.png}  % Adjust width as needed
...
\begin{figure}[htbp]  % Placement: Here, Top, Bottom, Page
    \centering
    \includegraphics[width=0.50\textwidth]{1NN2.png}  % Adjust width as needed
...
Narrow magnetic stripes are present, with their ordering changing at the "magical doping" value. The effect of including a next-nearest neighbour hopping parameter is examined in detail. Increasing $t'$ lowers the Van Hove singularity's energy, shifting the region of prominent SDW-SC interplay towards more negative chemical potentials.
...
\bibliographystyle{unsrturl}  % Choose a style such as plain, alpha, abbrv, etc.
