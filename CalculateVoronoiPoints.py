...
def plot_U_J_Voronoi(data,inputname,outputname):
    ...
    for i in range(int(len(sorted_lines)/10)): #take 10% of the longest lines and write those into the list. 
            x = (sorted_lines[i][0, 0] + sorted_lines[i][1, 0])/2.0
            y = (sorted_lines[i][0, 1] + sorted_lines[i][1, 1])/2.0
            if x > 0 and y > 0:
